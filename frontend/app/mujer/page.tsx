...
import { Product, WOMEN_CATEGORIES } from '@/types';
import { getProducts } from '@/lib/api';
import ProductCard from '@/components/ProductCard';
import CategoryFilter from '@/components/CategoryFilter';

/** Catálogo exclusivo de Mujer */
export default function MujerPage({
  ...
  useEffect(() => {
    ...
    getProducts()
      .then((all) => {
        // Filtrar: excluir categorías exclusivas de hombre
        const menOnly = ['camisa', 'corbata', 'traje'];
        const women = all.filter((p) => {
          const cat = (p.category ?? '').toLowerCase();
          return !menOnly.some((m) => cat === m); // coincidencia exacta o incluye
        });
        setProducts(women);
      })
      .catch(() => setError(true))
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      >
        {/* Decorative line — gold para mujer */}
        <div
          style={{
            position: 'absolute',
            left: 0,
        ...
        <div className="container-us">
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '16px', maxWidth: '480px', lineHeight: 1.6 }}>
            Moda femenina de alta costura que celebra la sofisticación, la modernidad y la feminidad sin límites.
          </p>
